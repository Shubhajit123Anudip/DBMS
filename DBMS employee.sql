...
create table EMP3(id int primary key auto_increment, name varchar(10) not null, Basic float(6,2) not null ,
...
#EMP1 table.
alter table emp3 modify basic int(20);
...
create table EMP_traine(Emp_id int primary key auto_increment, name varchar(10) not null, Basic double(6,2) not null ,
designation varchar(30) not null,age int(10));
#5. Insert following data in EMP3 table: -
...
INSERT INTO EMP_traine (Emp_id,Name,Basic,Designation,Age) select ID,Name,Basic,Designation,
Age FROM EMP3 WHERE Designation = 'Trainee';
#7. Display the structure of both the tables.
select * from emp3,emp_traine;
...
update EMP3 set Skills='java' where id=1;
update EMP3 set Skills='c' where id=2;
update EMP3 set Skills='c++' where id=3;
update EMP3 set Skills='python' where id=4;
update EMP3 set Skills='DBMS' where id=5;
update EMP3 set Skills='PHP' where id=6;
alter table EMP3 ADD DOJ varchar(10) after skills;
update EMP3 set DOJ='1/11/22' where id=1;
update EMP3 set DOJ='2/11/22' where id=2;
update EMP3 set DOJ='1/11/22' where id=3;
update EMP3 set DOJ='3/11/22' where id=4;
update EMP3 set DOJ='2/11/22' where id=5;
update EMP3 set DOJ='3/11/22' where id=6;
#9. Change the designation of all trainees in EMP_trainee table to ‘Programmer _Trainee’.
update EMP_traine set designation='Programmer_trainee';
#10. Update more than one row in one query in EMP1 table.
update emp3 set name ='Ram'  where id=1 or id=2;

#11.Change the data type of ID in EMP1 table to varchar2 and increase the data size to 5
alter table emp3 modify id varchar(5);
#12.Display both the tables EMP1 and EMP_trainee.
select * from emp3,EMP_traine;

#13.Rename the column Age of EMP1 table to Age_in_Years
alter table emp3 rename column age to age_in_years;
#14.Delete the details of all the trainees from the EMP1 table.
delete from emp3 where designation='trainee';
#15.Drop the Age column from the EMP_trainee table.
alter table emp_traine drop column age;
#16.Drop two columns in one query from EMP_trainee tabl
alter table emp_traine drop column basic,drop column designation;
#17.Rename the table EMP to EMP_Mgr_Engr.
alter table emp3 rename to EMP_Mgr_Engr;
#18.Drop the table EMP_Trainee. 
drop table  emp_traine;
